jQuery(function($) {
	...
	************************************************/
	    ...
	    $.fn.sao_coloris = function() { 	
	
				rang_Coloris({el: '.sao-coloris',swatches: [
...
function animatePieChart() {
	...
	************************************************/
 	function sao_fold_hide(items){ 
 	 	...
 	 	$(this).parents('.sao_options_warp').find('.sao_options_fold').each(function() {
					...
					var data_active  = $('.sao_id_'+data_name).attr('data-active');
 					if(	 data_active === 'show' ){
			...
			$(this).parents('.sao_options_warp').find('.sao_options_fold_item').each(function() {
				var data_name = $(this).attr('data-name');
				var actives  = $('.sao_id_+'+data_name).attr('data-active');
				if(	 actives === 'hide' ){
		...
		$(document).on("click",'.sao_title_tabs a',function() {
          ...
          $('.sao_title_tabs a:first-child').trigger("click");
 }, 10);
	...
	************************************************/
 	...
 	function sao_add_editor(classs = false){ 
     // فراخوانی wp_editor با Ajax
    $.ajax({
        url: ajaxurl,
        type: 'POST',
        data: {
            action: 'reload_editor_tabs'
        },
        success: function(response) {
            // نمایش wp_editor در یک عنصر مشخص
           classs.html(response);
		        tinymce.execCommand('mceAddEditor', false, 'ajax_wp_editor');
        		quicktags({id : 'ajax_wp_editor'});
        }
    });
	...
	************************************************/
 	function sao_add_hander(classs = false){ 
		...
		$(classs).find('.sao-editor-hander').each(function() {
			...
			tinymce.execCommand('mceAddEditor', false, id_name);
			quicktags({id :id_name});
	...
	***************************************************/
	...
	$(document).on('click', '.sao_builder_options_element .sao_options_update', function(e) {
		...
		$('.sao-editor-hander').each( function() {
			var data_attr =  $(this).attr('data-id');
			var editorContent = tinyMCE.get(data_attr).getContent();
  			var tinymce =  $(this).find('.wp-editor-area').val(editorContent);

<?php
if ( !function_exists ( "sao_builder_section" )){
...
function sao_builder_section($section_key=false,$section_value=false){
 	...
 	if(!empty($_REQUEST['template_id'])){
		$template =	get_option( 'sao_'.sanitize_text_field($_REQUEST['row_id']).'_template');
 		$column = sao_options_array_row(urldecode($template[sanitize_key($_REQUEST['template_id'])]['column']));
	...
	$key = !empty($_REQUEST['key']) ? sanitize_key($_REQUEST['key']): $section_key;
	...
	echo '<li id="sao_section_'.esc_attr($key).'" class="sao_section_item sao_builder_section_item  sao_row_item '.esc_attr($template_id).'" data-collapse="'.esc_attr($collapse).'" data-key="'.esc_attr($key).'" data-value="'.esc_attr($value['value']).'"  data-option="'.esc_attr($value_option).'" >';
		...
		echo '<div class="sao_row_content sao_section_content">';
			...
			echo '<div class="sao_row_bottom">';
				echo '<a class="sao_add_column sao_add_row">'.esc_html__('Add New Column','saoshyant-page-builder').'</a>';
				echo '<a class="sao_column_template_add sao_template_add" data-row="column"  data-name="'.esc_html__('Column','saoshyant-page-builder').'" ></a>';
			echo '</div>';
...
if ( !function_exists ( "sao_section_value" )){
function sao_section_value($key,$value){
	 
	echo '<div class="sao_row_title sao_section_title">';
    	echo '<span class="sao_section_name sao_row_name">'.esc_html__('Section','saoshyant-page-builder').'</span>';
		...
		echo '<a class="sao_row_template_save sao_template_save" data-row="section"   data-name="'.esc_html__('Section','saoshyant-page-builder').'"></a>';
	echo '</div>';
 
}
}
if ( !function_exists ( "sao_section_tabs" )){
...
if ( !function_exists ( "sao_section_options" )){
...
function sao_section_options(){
	...
	echo '<form id="sao_options_'.esc_attr(sanitize_key($_REQUEST['key'])).'" class="sao_options sao_options_section sao_active " data-key="'.esc_attr(sanitize_key($_REQUEST['key'])).'">';
	echo '<div class="sao_options_middle">';
		
		//Title
		echo '<div class="sao_options_title"><h3>'.esc_html__('Section Options','saoshyant-page-builder').'</h3><i class="sao_options_close"></i>';
...
if ( !function_exists ( "sao_section_template_remove" )){
...
function sao_section_template_remove() {
	...
	unset($old_data[sanitize_title($_REQUEST['key'])]);

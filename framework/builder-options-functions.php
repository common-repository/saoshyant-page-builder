...
*********************************************************************/
if ( !function_exists ( "sao_options_function" )){
...
function sao_options_function($value,$label,$id,$type, $options = false,$description = false,$placeholder = false,$fold=false) { 
	...
	elseif($type =='according-start'){
		...
		if( $type == 'tabs' ) {
			echo '<a class="sao_add_tab button" data-id="'.esc_attr($data).'" data-option="'.esc_attr(urlencode(serialize($options))).'" >'.esc_html__('Add Tab','saoshyant-page-builder');
  			echo '</a>';
			 
		}	
		if( $type == 'list' ) {
			echo '<a class="sao_add_list button" data-id="'.esc_attr($data).'" data-option="'.esc_attr(urlencode(serialize($options))).'" >'.esc_html__('Add Item','saoshyant-page-builder');
  			echo '</a>';;
		}			
	...
	//Color		
	case 'color':	
 		echo '<input  class="sao-color sao-coloris"  data-rgba="false" type="text"   name="'.esc_attr($data).'" id="sao_label_'.esc_attr($data).'" value="'.esc_attr($value_id).'">';
  	break;
	
	//Color RGBA
	case 'color_rgba':
		echo '<input  class=" sao-color  sao-coloris" data-rgba="true" type="text"   name="'.esc_attr($data).'" id="sao_label_'.esc_attr($data).'" value="'.esc_attr($value_id).'">';
 	break;
    	...
    	echo '<div class="sao_dropdown">';
			echo '<dt>'; 
				echo '<a>'; 
					...
					echo '<span class="sao_hida" style="'.esc_attr($display).'">'.esc_html__('Select','saoshyant-page-builder').'</span>'; 
	...
	case 'icon': 
		$icon_rand = rand(10000000, 99999999);
  		echo'<div  class="sao_menu_icon" id-icon="'.esc_attr($icon_rand).'" >';
 			echo'<a  class="sao_builder_choose_icon button button-small" >'.esc_html__('Choose Icon','saoshyant-page-builder').'</a>';
	...
	// Image
	case 'image': 
	  	echo'<a class="sao_image_upload button button-small"  data-name="'.esc_attr($data).'"  >'.esc_html__('Upload','saoshyant-page-builder').'</a>';
		if(!empty($value_id)){
	...
	case 'list':
  	...
  	//Gallery
  	case 'gallery': 
	  	echo'<a class="sao_add_image button button-small" data-name="'.esc_attr($data).'"  >'.esc_html__('Add Image','saoshyant-page-builder').'</a>';
...
add_action('wp_ajax_nopriv_sao_options_function', 'sao_options_function');
 if ( !function_exists ( "sao_options_function_multi_item" )){		 

function sao_options_function_multi_item($id,$value,$options,$fold=false) {
 		if(!empty($options)){
		...
		$option_type = !empty( $option_value['type'] ) ?  $option_value['type']  : '';
			...
			$option_id =!empty($option_value['id'])?$option_value['id']:'';
				...
				switch( $option_type ) {
					...
					//Color	
					case 'color':
						echo '<input  class="  sao-color sao-coloris"  data-rgba="false" type="text"   name="'.esc_attr($data).'" id="sao_label_'.esc_attr($data).'" value="'.esc_attr($value_content).'">';
					break;
						
					//Color RGBA
					case 'color_rgba':
						echo '<input  class=" sao-color sao-coloris" data-rgba="true" type="text"   name="'.esc_attr($data).'" id="sao_label_'.esc_attr($data).'" value="'.esc_attr($value_content).'">';
					break;
 ...
 if ( !function_exists ( "sao_options_function_tabs_item" )){
...
function sao_options_function_tabs_item($id_tabs=false,$value_tabs=false,$vkey=false,$options =false) { 
	...
	$key = !empty($_REQUEST['key_tabs']) ? sanitize_key($_REQUEST['key_tabs']): $vkey;
		...
		echo '<ul class="sao_multi_options">'; 
				...
				foreach ($option as  $option_value) { 	
					...
					//Color RGBA
					case 'color_rgba':
						echo '<input  class=" sao-color sao-coloris" data-rgba="true" type="text"   name="'.esc_attr($data).'" id="sao_label_'.esc_attr($data).'" value="'.esc_attr($value).'">';
					break;
					...
					case 'icon': 
							...
							echo'<a  class="sao_builder_choose_icon button button-small"   data-id="sao_'.esc_attr($data).'" >'.esc_html__('Choose Icon','saoshyant-page-builder').'</a>';
 ...
 if ( !function_exists ( "sao_options_function_list_item" )){
		...
		echo'<div class="sao_list_options_warp">';
			...
			foreach ($option as  $option_value) :
				...
				$placeholder = !empty( $option_value['placeholder'] ) ?  $option_value['placeholder']  : null;
					 ...
					 $option_id =!empty($option_value['id'])?$option_value['id']:'';
					$value_id = isset($value[$option_id])?$value[$option_id]:'';
 				$data = $id.'['.$key.']['.$option_value['id'].']';
				...
				if(!empty($option_value['fold'])){
					...
					echo '<section class="sao_options_setting_2  ">';
 					switch( $option_type ) {
						...
						//Color
						case 'color':
							echo '<input  class="sao-coloris sao-color"  data-rgba="false" type="text"   name="'.esc_attr($data).'" id="sao_label_'.esc_attr($data).'" value="'.esc_attr($value_id).'">';
						break;
...
function sao_icon_picker() {
		...
		echo '<div class="sao_icon" data-id="'.esc_attr(sanitize_title($_REQUEST['id'])).'">';
						echo '<div class="sao_icon_middle">';
							echo '<div class="sao_icon_title">'.esc_html__('Choose Icon','saoshyant-page-builder').'<i class="sao_icon_close"></i><div class="sao_icon_search"><input id="search-input" class="search-icon-control" placeholder="'.esc_html__('Search icons','saoshyant-page-builder').'" autocomplete="off" spellcheck="false" autocorrect="off" tabindex="1"></div>';
							...
							echo '<div class="sao_icon_bottom"><a class="sao_set_icon button-primary">'.esc_html__('Set Icon','saoshyant-page-builder').'</a></div>';	 
						echo'</div>';

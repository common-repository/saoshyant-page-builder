<?php
 if ( !function_exists ( "sao_builder_column" )){
...
function sao_builder_column($column_key=false,$column_value=false){
 	...
 	if(!empty($_REQUEST['value'])){
		...
		$value['child'] = sanitize_key($_REQUEST['child']);
	...
	$key = !empty($_REQUEST['key']) ? sanitize_key($_REQUEST['key']): $column_key;
	...
	echo '<li id="sao_column_'.esc_attr($key).'" class="sao_column_item sao_builder_column_item sao_column_'.esc_attr($value['value']).' '.esc_attr($template_id).' sao_row_item"  data-collapse="'.esc_attr($collapse).'" data-key="'.esc_attr($key).'" data-value="'.esc_attr($value['value']).'"  data-option="'.esc_attr($value_option).'"  data-child="'.esc_attr($value['child']).'">';
		...
		echo '<div class="sao_row_content sao_column_content">';
			...
			echo '<div class="sao_row_bottom">';
				echo '<a class="sao_add_element sao_add_row">'.esc_html__('Add New Element','saoshyant-page-builder').'</a>';
				echo '<a class="sao_element_template_add sao_template_add"  data-row="element" data-name="'.esc_html__('Element','saoshyant-page-builder').'"></a>';
			echo '</div>';
...
if ( !function_exists ( "sao_column_value" )){
...
function sao_column_value($key,$value ) {
	...
	echo '<div class="sao_row_title sao_column_title">';
		...
		echo '<a class="sao_row_template_save  sao_template_save" data-row="column" data-name="'.esc_html__('Column','saoshyant-page-builder').'" ></a>';
    
 	echo '</div>';
   
}
}
if ( !function_exists ( "sao_column_tabs" )){
...
if ( !function_exists ( "sao_column_options" )){
...
function sao_column_options() {
	...
	echo '<form id="sao_options_'.esc_attr(sanitize_key($_REQUEST['key'])).'" class="sao_options sao_options_column sao_active " data-key="'.esc_attr(sanitize_key($_REQUEST['key'])).'">';
...
if ( !function_exists ( "sao_model_column" )){
function sao_model_column() {
	...
	echo '<div class="sao_model_middle">';
		//Title
		echo '<div class="sao_model_title"><h3>'.esc_html__('Add New Column','saoshyant-page-builder').'</h3><i class="sao_model_close"></i></div>';
		//Content
 ...
 if ( !function_exists ( "sao_builder_column_list" )){
...
function sao_builder_column_list() {
	...
	$template =	get_option( 'sao_'.sanitize_title($_REQUEST['row_id']).'_template');
  	$column = sao_options_array_row(urldecode($template[sanitize_title($_REQUEST['template_id'])]['column']));

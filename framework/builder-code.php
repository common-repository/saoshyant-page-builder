...
sao Options Encode
*********************************************************************/
 if ( !function_exists ( "sao_sanitize_text_or_array_field" )){

function sao_sanitize_text_or_array_field($array_or_string) {
    if( is_string($array_or_string) ){
        $array_or_string = wp_kses_post($array_or_string);
    }elseif( is_array($array_or_string) ){
        foreach ( $array_or_string as $key => &$value ) {
            if ( is_array( $value ) ) {
                $value = sao_sanitize_text_or_array_field($value);
            } 
            else {
...
if ( !function_exists ( "sao_options_encode" )){
...
function sao_options_encode($opt=false){
	...
	if(!empty($opt)){
  		...
  		$option = sao_sanitize_text_or_array_field($_REQUEST['option']);
...
sao Options DeCode
*********************************************************************/
  if ( !function_exists ( "sao_options_decode" )){

function sao_options_decode($data){
 	$o = sao_serialize_code(stripslashes( $data),'sao');
	$a =array();
	//OPTION1
	if(is_array($o)){
	foreach($o as $v) :
	 	$a[stripslashes($v['key'])] = sao_options_decode_array($v);
	endforeach;
	}
	return $a;
}
  }
  if ( !function_exists ( "sao_options_decode_array" )){

 function sao_options_decode_array($v){
	 $a = array();
	 $o_2 = sao_serialize_code(stripslashes($v['value']),stripslashes($v['name']));
		if(is_array($o_2)){
		foreach($o_2 as $v_2) :
			 
 		  		$a[urldecode(stripslashes($v_2['key']))] = sao_options_decode_array($v_2);
...
*********************************************************************/
  if ( !function_exists ( "sao_options_array_row" )){
